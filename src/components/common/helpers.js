...
export function isDateString(value) {
    ...
    return Date.parse(value); // value.match(/^\d{2}-\d{2}-\d{4}$/)
}
...
export function columnAlign(value) {
...
export function filterRows(rows, filters) {
    ...
    return rows.filter((row) => {
        return Object.keys(filters).every((fieldname) => {
            ...
            const searchValue = filters[fieldname]

            if (isDateString(value) && isDateString(searchValue)) {
                var date1 = Date.parse(value.split(' ')[0].replaceAll('-', '/'))
                var date2 = Date.parse(searchValue.split(' ')[0].replaceAll('-', '/'))
                return date1 === date2;
            }
